...
namespace NESterpiece
{
	...
	void InputMenu::draw_keyboard_map(InputBindingProfile &map)
	{
		...
		if (BeginTable("Keyboard Bindings", 2, ImGuiTableFlags_SizingFixedFit))
		{
			...
			if (draw_input_button("Left Button", map.left) && !input_key_modal)
			{
				...
				input_key_target = StdControllerButton::Left;
			}
			TableNextColumn();
			AlignTextToFramePadding();
			Text("A");
			...
			{
				input_key_modal = true;
				input_key_target = StdControllerButton::A;
			}
			TableNextRow();
			TableNextColumn();
			AlignTextToFramePadding();
			...
			{
				input_key_modal = true;
				input_key_target = StdControllerButton::Right;
			}
			TableNextColumn();
			AlignTextToFramePadding();
			Text("B");
			...
			{
				input_key_modal = true;
				input_key_target = StdControllerButton::B;
			}
			TableNextRow();
			TableNextColumn();
			AlignTextToFramePadding();
			...
			{
				input_key_modal = true;
				input_key_target = StdControllerButton::Up;
			}
			TableNextColumn();
			AlignTextToFramePadding();
			Text("Select");
			...
			{
				input_key_modal = true;
				input_key_target = StdControllerButton::Select;
			}
			TableNextRow();
			TableNextColumn();
			AlignTextToFramePadding();
			...
			{
				input_key_modal = true;
				input_key_target = StdControllerButton::Down;
			}
			TableNextColumn();
			AlignTextToFramePadding();
			Text("Start");
			...
			{
				input_key_modal = true;
				input_key_target = StdControllerButton::Start;
			}
			EndTable();
		}
	}
	...
	void InputMenu::update_key_mapping(StdControllerButton btn, InputSource source)
	{
		auto &config = Configuration::get();
		auto &map = config.input_profiles[selected_binding];
		switch (btn)
		{
		case StdControllerButton::Left:
			map.left = std::move(source);
			break;
		case StdControllerButton::Right:
			map.right = std::move(source);
			break;
		case StdControllerButton::Up:
			map.up = std::move(source);
			break;
		case StdControllerButton::Down:
			map.down = std::move(source);
			break;
		case StdControllerButton::A:
			map.a = std::move(source);
			break;
		case StdControllerButton::B:
			map.b = std::move(source);
			break;
		case StdControllerButton::Select:
			map.select = std::move(source);
			break;
		case StdControllerButton::Start:
